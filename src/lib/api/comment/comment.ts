...
import { CommentDTO, CommentRequestDTO } from '../../types/comment';
...
class TestModeStorage {
  ...
  public updateComment(commentId: number, content: string, mentions: string[]): CommentDTO | undefined {
    let targetComment: CommentDTO | undefined;
    
    // 먼저 최상위 댓글에서 찾기
    targetComment = this.comments.find(comment => comment.commentId === commentId);
...
export const getComments = async (reviewId: number, isTestMode = false): Promise<CommentDTO[]> => {
  if (isTestMode) {
...
export const createComment = async (request: CommentRequestDTO, isTestMode = false): Promise<CommentDTO> => {
  if (isTestMode) {
...
export const updateComment = async (commentId: number, request: CommentRequestDTO, isTestMode = false): Promise<CommentDTO> => {
  if (isTestMode) {
    const storage = TestModeStorage.getInstance();
    const updatedComment = storage.updateComment(commentId, request.comment, request.mentions || []);
    if (!updatedComment) {
...
export const deleteComment = async (commentId: number, isTestMode = false): Promise<void> => {
  if (isTestMode) {
