...
// 테스트 모드용 함수들
export const getTestComments = async (reviewId: number): Promise<CommentDTO[]> => {
  return TestModeStorage.getInstance().getComments(reviewId);
};

export const createTestComment = async (request: CommentRequestDTO): Promise<CommentDTO> => {
