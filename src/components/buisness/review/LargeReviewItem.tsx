...
const LargeReviewItem: React.FC<LargeReviewItemProps> = ({ reviewId }) => {
  const [ReviewItemResponseDto, eedReviewItemResponseDto] =
    useState<ReviewItemResponseDto | null>(null);
  const router = useRouter();

  useEffect(() => {
    fetch(`http://localhost:8080/review-detail/${reviewId}`)
      .then((res) => res.json())
      .then((data) => eedReviewItemResponseDto(data))
      .catch((err) =>
        console.error('리뷰 데이터를 가져오는 중 오류 발생:', err)
      );
  }, [reviewId]);

  if (!ReviewItemResponseDto) {
    return <div className="text-center p-4">리뷰를 불러오는 중...</div>;
  }

  const handleNavigate = () => {
    router.push(`/review-detail/${ReviewItemResponseDto.reviewId}`);
  };
    ...
    >
      <div className="flex flex-col flex-1 justify-between pr-4">
        ...
        <h2 className="text-lg font-semibold text-gray-800">
          {ReviewItemResponseDto.title}
        </h2>
        <p className="text-sm text-gray-600 line-clamp-1">
          {ReviewItemResponseDto.content}
        </p>
        <p className="text-xs text-gray-500">
          {ReviewItemResponseDto.userDataResponse.nickname}
        </p>

        <div className="flex flex-row space-x-2 mt-4">
          {ReviewItemResponseDto.imageUrls.map((url, index) => (
            <img
              key={index}
              src={url}
              alt="리뷰 이미지"
      ...
      <div className="flex flex-row m-0 border border-gray-300">
        <img
          src={ReviewItemResponseDto.thumbnailUrl}
          alt="웹툰 썸네일"
